...
int sdmain(void) {
	uint8_t *ram = (uint8_t*)0x80000000;
	uint8_t *bin_addr = ram + (1 << 20);
	int argc; char **argv;
	fatdata_t fatdata;
	fat_entry_t *p;
	...
	SDIO_VERBOSITY(0);
	{
		uint8_t *mem = (uint8_t*)(ram + 0x8000);
		fatdata.buf = mem;
	}
	...
	}
	{
		...
		if (size > (15 << 20) - 0x10000) {
			printf("binary is too big\n");
			return 1;
		}
		n = fat_read_simple(&fatdata, clust, bin_addr, size);
		printf("read = 0x%x\n", n);
		if (n < size) {
			printf("fat_read failed\n");
			return 1;
		}
		if (MEM4(bin_addr) != 0x42544844) {
			printf("no DHTB header\n");
			return 1;
		}
	}
	{

...
// must be run from on-chip RAM
#ifdef SMC_INIT_BUF
	(void)a; (void)ps; (void)base;
	{
		int *p = sc6531da_init_smc_asm;
		int n = sc6531da_init_smc_asm_end - p;
		int *d = (int*)SMC_INIT_BUF, *f = d;
		do *d++ = *p++; while (--n);
		d[0] = ps_off1 << 1; d[1] = ps_off2 << 1;
		d[2] = freq_div | conf0 << 8;
		d[3] = smc08; d[4] = smc00; d[5] = smc20;
		d[6] = smc24; d[7] = smc04;
		d[8] = smc50; d[9] = smc54; d[10] = smc58; d[11] = smc5c;
		((void(*)(void))(intptr_t)f)();
	}
#else
